...
val alberta_education_curriculum = 
  ...
  .countItems() 
  .saveAsTextFile("/data/derivatives/urls/alberta_education_curriculum")

val alberta_education_curriculum_francais = 
  RecordLoader.loadArchives("/data/alberta_education_curriculum_francais/*.gz", sc) 
  .keepValidPages() 
  .map(r => (r.getCrawlMonth, ExtractDomain(r.getUrl))) 
  .countItems() 
  .saveAsTextFile("/data/derivatives/urls/alberta_education_curriculum_francais")
